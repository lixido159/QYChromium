...
bool proptyFunc_View_Left(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setLeft(YGPointValue(propValue->getNumberValue()));
    return true;
};

bool proptyFunc_View_Top(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setTop(YGPointValue(propValue->getNumberValue()));
    return true;
};

bool proptyFunc_View_Height(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setHeight(parseNumberStringToYGValue(propValue->getStringValue()));
    return true;
};

bool proptyFunc_View_Width(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setWidth(parseNumberStringToYGValue(propValue->getStringValue()));
    return true;
};
...
bool proptyFunc_View_Display(IQYBaseView *view, QYPropertyValue *propValue) {
    YGDisplay display = propValue->getStringValue().compare("flex") == 0 ? YGDisplayFlex : YGDisplayNone;
...
bool proptyFunc_View_Flex_Direction(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setFlexDirection(parseFlexDirectionCssValue(propValue->getStringValue()));
    return true;
};

bool proptyFunc_View_Flex_Wrap(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setFlexWrap(parseFlexWrapCssValue(propValue->getStringValue()));
    return true;
};
...
bool proptyFunc_View_Justify_Content(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setJustifyContent(parseJustifyContentCssValue(propValue->getStringValue()));
    return true;
};
...
bool proptyFunc_View_Align_Items(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setAlignItems(parseAlignItemsCssValue(propValue->getStringValue()));
    return true;
};

//子元素在交叉轴对齐方式，flex-wrap设置后生效
bool proptyFunc_View_Align_Content(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setAlignContent(parseAlignContentCssValue(propValue->getStringValue()));
    return true;
};
...
bool proptyFunc_View_Flex_Shrink(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setFlexShrink(propValue->getNumberValue());
    return true;
};

//主轴的大小，如果row就是宽度，column就是高度
bool proptyFunc_View_Flex_Basis(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setFlexBasis(parseNumberStringToYGValue(propValue->getStringValue()));
    return true;
}
...
bool proptyFunc_View_Align_Self(IQYBaseView *view, QYPropertyValue *propValue) {
    view->getNodeLayout()->setAlignSelf(parseAlignItemsCssValue(propValue->getStringValue()));
    return true;
}
